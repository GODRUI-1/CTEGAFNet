    ...
    for _data_name in opt.data_lst:
        ...
        with open(filename, 'w+') as file_to_write:
            ...
            for _model_name in opt.model_lst:
                ...
                img_name_lst = [file for file in os.listdir(gt_src) if not file.startswith('.ipynb_checkpoints')]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=['micai_te'],
        # 'COD10K', 'NC4K'
        choices=['CAMO', 'COD10K', 'NC4K','micai_te'])
    parser.add_argument(
        '--model_lst', type=list, help='candidate competitors',
        # 'Exp-DGNet-S', 'Exp-DGNet', 'Exp-DGNet-PVTv2-B0', 'Exp-DGNet-PVTv2-B1', 'Exp-DGNet-PVTv2-B2','Exp-DGNet-PVTv2-B3'
        default=['CTEGAFNet_MICAI' ])
        ...
        for _data_name in opt.data_lst:
            for _model_name in opt.model_lst:
                ...
                pred_pth = os.path.join(opt.pred_root, _model_name, _data_name)
                            # 获取目录内容，但不包括.ipynb_checkpoints
                gt_contents = [f for f in os.listdir(gt_pth) if not f.startswith('.ipynb_checkpoints')]
                pred_contents = [f for f in os.listdir(pred_pth) if not f.startswith('.ipynb_checkpoints')]
            
                if not sorted(gt_contents) == sorted(pred_contents):
                    print(len(sorted(gt_contents)), len(sorted(pred_contents)))
