...
def train(train_loader, model, optimizer, epoch, save_path, writer):
    ...
    try:
        for i, (images, gts, grads, edges) in enumerate(train_loader, start=1):
            ...
            pre,grad,l1,l2,l3,l4,ldice = preds
            loss1 = structure_loss(l1, gts)
            loss2 = structure_loss(l2, gts)
            loss3 = structure_loss(l3, gts)
            loss4 = structure_loss(l4, gts)
            losse = dice_loss(ldice, edges)
            
            loss_out = loss1 + loss2 + loss3 + loss4
            loss_the = loss_out + losse
            loss_pred = structure_loss(pre, gts)
            loss_grad = grad_loss_func(grad, grads)
            loss = loss_pred + loss_grad + loss_the
            loss.backward()
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} '
                      'loss_pred: {:.4f} loss_grad: {:0.4f} ' 'loss_the: {:.4f} losse: {:.4f} '.
                ...
                logging.info(
                    ...
                    format(datetime.now(), epoch, opt.epoch, i, total_step,
                             ...
                             # loss3.data,loss3: {:.4f} loss2: {:0.4f} loss1: {:0.4f} 
                             # loss2.data,
                             # loss1.data,
                # TensorboardX-Loss
                writer.add_scalars('Loss_Statistics',
                                   {'loss_pred': loss_pred.data, 'loss_grad': loss_grad.data,'loss_the':loss_the.data,
                                    'Loss_total': loss.data,'losse':losse.data
                                    },
                                   global_step=step)
...
def val(test_loader, model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        ...
        cur_score = 1.5*metrics_dict['Sm'] + metrics_dict['mxFm'] + metrics_dict['mxEm']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_id', type=str, default='0',
                        help='train use gpu')
    parser.add_argument('--save_path', type=str, default='./snapshot/CTEGAFNet_MICAI/',
    ...
    #     model = Network(channel=64, arc='PVTv2-B2', M=[8, 8, 8], N=[4, 8, 16]).cuda()
        ...
        model = Network(channel=64, arc='Convnext', M=[8, 8, 8, 8], N=[1, 4, 8, 16]).cuda()
